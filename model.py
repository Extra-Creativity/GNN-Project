...
def get_graph_feature_with_centroid(x, k):
    batch_size = x.size(0)
    num_dims = x.size(1)
    num_points = x.size(2)
    x = x.view(batch_size, -1, num_points)
    idx = knn(x, k = k)
    device = torch.device('cuda')
    centroid = torch.mean(x, dim=2)
    centroid = centroid.view(batch_size,-1,1).repeat_interleave(num_points, dim = 2)
    x = x - centroid
    ...
    feature = x.view(batch_size * num_points, -1)[idx, :]
    feature = feature.view(batch_size, num_points, k, num_dims)
    diff = feature - x.view(batch_size, num_points, 1, num_dims).repeat(1, 1, k, 1)
    x = F.normalize(x, p=2, dim = 2)
    x = x.view(batch_size, num_points, 1, num_dims).transpose(2,3)
    centroid_feature = torch.matmul(diff, x).view(batch_size, num_points, -1)
    return centroid_feature.transpose(1,2)
...
class DGCNN_CDF(nn.Module):
    def __init__(self, args, output_channels=40):
        ...
        self.bn4 = nn.BatchNorm2d(256)
        self.bn5 = nn.BatchNorm1d(args.emb_dims)
        self.conv1 = nn.Sequential(nn.Conv2d(46, 64, kernel_size=1, bias=False),
                                   self.bn1,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv2 = nn.Sequential(nn.Conv2d(64*2, 64, kernel_size=1, bias=False),
                                   self.bn2,
    ...
    def forward(self, x):
        batch_size = x.size(0)
        cdfvec = get_graph_feature_with_centroid(
            x, self.k)
        x = torch.concat((x, cdfvec), dim=1)
        x = get_graph_feature(x, k=self.k)
        x = self.conv1(x)
        x1 = x.max(dim=-1, keepdim=False)[0]
